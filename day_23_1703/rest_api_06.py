...
@app.get("/client/{id}")
def fn_client_id(id: int):
    info = client_info(DB, id)
    return info


@app.get("/client_update/{id}")
def fn_client_update_id(id: int, new_salary: int):
    if client_salary_update(DB, id, new_salary):
        return {'temp_id': id,
                'temp_salary': new_salary}
    else:
